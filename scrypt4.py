...
def json_reading(json_parser, json_products):
    '''
    Функция для работы с бэкендом,
    принимает на вход данные в формате json как переменные
    возвращает эти же данные в формате DF
    '''
...
def csv_reading(path=''):
    ...
    products = pd.read_csv(path + 'marketing_product.csv', sep=';').drop(['Unnamed: 0'], axis=1)
    return parser, products
